...
public class GameCommunicator extends JavaGameCommunicator {
	...
	public Variant performGameFunction(ApiCall call, short aiId) {
		...
		case API_TEST_INT_MATRIX_SUM:
			return new Variant(gameInstance.testIntMatrixSum(
				(int[]) call.getParameter(0).getValue()));
			
		case API_TEST_INT_MATRIX_ADD:
			return new Variant(gameInstance.testIntMatrixAdd(
				(int[][][]) call.getParameter(0).getValue(),
		...
		case API_TEST_CALLBACK:
			gameInstance.testCallback(
				(Integer) call.getParameter(0).getValue());
			return new Variant();
